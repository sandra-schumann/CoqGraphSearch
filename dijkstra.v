...
(* inlining dijkstra_stepdid NOT give us functional induction, but
   separating it out did... *)
Definition dijkstra_step (f:node->node->nat)
  ...
  | Some p => let (found_u, frontierRemaining) := p in
      ...
      let frontierNew := map (fun v => (v, (Some u, f u v+l))) neighbors in
...
Ltac expandDijkstra := 
    match goal with
      [ H : context[dijkstra_step ?m ?g ?u ?f ?p] |- _ ]
          =>unfold dijkstra_step in H
...
Function dijkstra' f
  ...
  : list found
  :=
  match dijkstra_step f g unexpanded frontier parent with
  | None => parent
  | Some args =>
...
Lemma in_neigh_in_map : forall {A B:Type} (w:A) (f:A->B) neigh,
  In w neigh -> In (w,f w) (map (fun w => (w,f w)) neigh).
Proof.
...
Lemma in_neigh_in_frontier' : forall {B} f fr (g:node->B) neigh fr' v,
  In v neigh ->
  fold_right (insert f) fr (map (fun w => (w,g w)) neigh) = fr' ->
  In v (keys fr').
Proof.
...
.
  ...
  destruct p; destruct f0; myinj' Heqc; destruct p).

  {
        ...
        [|specialize (remove_preserves _ _ _ HunepandedRemove d); crush]].
    {
      ...
      eelim (HextendFrontier _ v u HhdOut unexpanded HwvUnexpanded).
      {
        ...
        - rewrite <- HfrontierInsert. exists (f u v' + (foundPathLen (u, pu))).
...

  {
    ...
    destruct upptr as [u_parent|]. {
      ...
      - replace (        pathLength f (v :: u :: u_parent_path))
           with (f u v + pathLength f (     u :: u_parent_path)) by admit (* TODO *).
        replace (pathLength f (u :: u_parent_path)) with ul by auto.
...
Section ex1. (* Dijkstra example from CLRS 3rd edition *)
  Notation r := (Node 0).
  Notation s := (Node 1).
  Notation t := (Node 2).
  Notation u := (Node 3).
